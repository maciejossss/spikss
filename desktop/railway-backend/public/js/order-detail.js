...
const OrderDetail = {
  ...
  setup(props, { emit }) {
    ...
    // Format bez new Date – zachowuje godzinę z ISO
    const formatDateSafe = (s) => {
      try {
        if (!s) return '';
        const str = String(s);
        if (str.includes('T')) return str.slice(0,10) + ', ' + str.slice(11,16);
      ...
      <div class="bg-white shadow-sm border-b p-4">
        <div class="space-y-3">
          ...
          <div class="flex items-center text-gray-600">
            ...
            <span>{{ formatDateSafe(order.scheduled_date) }}</span>
          </div>
        </div>
      </div>
