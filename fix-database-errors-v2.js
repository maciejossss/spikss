...
db.serialize(() => {
  ...
  // 2. Napraw kolumnę brand w urządzeniach - POPRAWIONA WERSJA
  db.all("PRAGMA table_info(devices)", (err, columns) => {
    if (err) {
      console.error('❌ Błąd sprawdzania kolumn devices:', err);
      return;
    }
    ...
    if (!hasBrand) {
      ...
      db.run('ALTER TABLE devices ADD COLUMN brand TEXT', (err) => {
        if (err) {
          console.error('❌ Błąd dodawania kolumny brand:', err);
        } else {
          console.log('✅ Kolumna brand dodana do tabeli devices');
        }
      });
    } else {
  ...
  db.run(`
    ...
    WHERE id = 1
  `, (err) => {
    if (err) console.error('❌ Błąd aktualizacji klienta 1:', err);
    else console.log('✅ Klient 1 (Jan Kowalski) naprawiony');
  });
  ...
  db.run(`
    ...
    WHERE id = 2
  `, (err) => {
    if (err) console.error('❌ Błąd aktualizacji klienta 2:', err);
    else console.log('✅ Klient 2 (Maria Nowak) naprawiony');
  });
  ...
  db.run(`
    ...
    WHERE id = 3
  `, (err) => {
    if (err) console.error('❌ Błąd aktualizacji klienta 3:', err);
    else console.log('✅ Klient 3 (Piotr Wiśniewski) naprawiony');
  });
