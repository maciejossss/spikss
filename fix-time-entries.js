...
// Ścieżka do lokalnej bazy danych SQLite (Electron userData)
const userDataPath = app.getPath('userData');
const dbPath = path.join(userDataPath, 'serwis.db');

async function fixTimeEntriesTable() {
  const db = new sqlite3.Database(dbPath);
  
  return new Promise((resolve, reject) => {
    console.log('🔧 Naprawianie tabeli time_entries...');
...
fixTimeEntriesTable()
  .then(() => {
    console.log('✅ Naprawa tabeli time_entries zakończona');
  })
  .catch((error) => {
